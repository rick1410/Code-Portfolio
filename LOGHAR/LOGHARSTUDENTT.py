...
class LogHARStudentTModel:
    """
    Log-HAR model for realized volatility with Student-t errors on the log scale.

    We model y_t = log(RV_t). The conditional mean of y_t follows the HAR structure:
        E[y_t | F_{t-1}] = alpha0
                           + alpha1 * y_{t-1}
                           + alpha2 * mean(y_{t-5..t-1})
                           + alpha3 * mean(y_{t-22..t-1})

    The residuals e_t = y_t - E[y_t | F_{t-1}] are assumed Student-t with df=nu1
    and variance parameter sigma2 (scale = sqrt(sigma2)).

    Parameters
    ----------
    realized_volatility : array_like
        Positive realized-volatility series (we take logs internally).
    """

    model_name: str = "LHAR-t"
    distribution: str = "Student t"
    only_kernel: bool = True  # uses realized volatility only

    def __init__(self, realized_volatility: np.ndarray) -> None:
        rv = np.asarray(realized_volatility, dtype=float)
        if rv.ndim != 1:
            raise ValueError("realized_volatility must be a 1-D array.")
        if np.any(rv <= 0):
        ...
        self.log_likelihood_value: Optional[float] = None  # total (effective-sample) LL
    ...
    @staticmethod
    def _har_mean_equation(
        t: int, y: np.ndarray, a0: float, a1: float, a2: float, a3: float
    ) -> float:
        daily = y[t - 1]
    ...
    def log_likelihood(self, params: List[float]) -> float:
        """
        Average *negative* log-likelihood for Log-HAR with Student-t errors.

        Parameters
        ----------
        params : list
            [alpha0, alpha1, alpha2, alpha3, sigma2, nu1]
        ...
        """
        alpha0, alpha1, alpha2, alpha3, sigma2, nu1 = params

        # Parameter checks
        if sigma2 <= 0 or not np.isfinite(sigma2):
            return 1e10
        if nu1 <= 2 or not np.isfinite(nu1):
            return 1e10

        y = self.log_rv
        T = len(y)
        if T < 23:
            # need at least 22 obs to form weekly/monthly lags
            return 1e10

        resid = np.array(
            [y[t] - self._har_mean_equation(t, y, alpha0, alpha1, alpha2, alpha3)
             for t in range(22, T)],
            dtype=float
        )
        ...
        # Student-t log-density (location 0, scale sqrt(sigma2), df=nu1)
        const = gammaln((nu1 + 1) / 2.0) - gammaln(nu1 / 2.0) - 0.5 * np.log(nu1 * np.pi)
        ll = const - 0.5 * np.log(sigma2) - ((nu1 + 1) / 2.0) * np.log(1.0 + (resid**2) / (nu1 * sigma2))
    ...
    def compute_aic_bic(self, total_ll: float, num_params: int) -> Tuple[float, float]:
        """
        AIC/BIC from total (effective-sample) log-likelihood.

        Parameters
        ----------
        total_ll : float
            Sum of log-likelihood over effective sample (T-22).
        num_params : int
    ...
    def optimize(self,initial_params: Optional[Dict[str, float]] = None,compute_metrics: bool = False) -> Union[Dict[str, float], Tuple[Dict[str, float], float, float, float, np.ndarray]]:
        """
        Estimate parameters by SLSQP with simple bounds.

        Parameters
        ----------
        initial_params : dict or None
            Keys: alpha0, alpha1, alpha2, alpha3, sigma2, nu1.
            If None, uses previous MLE (if available) or a heuristic guess.
        ...
        if self.convergence:
            ...
            # Keep prior MLE if available, otherwise return the start
            return self.optimal_params if self.optimal_params is not None else dict(zip(keys, x0))

        if compute_metrics and self.convergence:
            # res.fun is the average NEGATIVE log-likelihood over the effective sample (T_eff = T-22)
            T_eff = max(0, len(y) - 22)
            total_ll = float(-res.fun * T_eff)  # convert to total (positive) log-likelihood
    ...
    def multi_step_ahead_forecast(self, horizon: int) -> np.ndarray:
        """
        Multi-step-ahead forecasts for realized volatility (original scale).

        We iterate the HAR recursion on the log scale and exponentiate.
