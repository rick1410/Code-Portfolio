...
class GARCHModel:
    ...
    optimal_params : Optional[Dict[str, float]]
        Fitted parameters if `optimize` succeeds with keys: {'omega','alpha','beta'}.
    ...
    def log_likelihood(self, params: Sequence[float]) -> float:
        """
        Average negative log-likelihood for GARCH(1,1) with Normal errors.
    ...
    def optimize(self, initial_params: Optional[Dict[str, float]] = None, compute_metrics: bool = False) -> Dict[str, float] | Tuple[Dict[str, float], float, float, float, np.ndarray]:
        """
        Fit parameters via SLSQP with positivity and stationarity constraints.

        Parameters
        ----------
        initial_params : dict or None, default None
            If None, uses previous optimum if available else defaults:
            {'omega': var(r[:50])*0.2, 'alpha': 0.1, 'beta': 0.7}.
        ...
        if initial_params is None:
            initial_params = self.optimal_params or {'omega': float(np.var(self.log_returns[:50]) * 0.2), 'alpha': 0.1, 'beta': 0.7}
        keys = list(initial_params.keys()); x0 = list(initial_params.values())
        n_obs = len(self.log_returns)

        bounds = [(1e-6, None), (0.0, None), (0.0, None)]
    ...
    def multi_step_ahead_forecast(self, horizon: int) -> np.ndarray:
        """
        Multi-step variance forecast via GARCH recursion.
        ...
        omega = self.optimal_params['omega']; alpha = self.optimal_params['alpha']; beta = self.optimal_params['beta']
        r = self.log_returns
        T = len(r)
        ...
        last = float(sigma2[-1])
        f1 = omega + alpha * (r[-1] ** 2) + beta * last
        forecasts = [f1]
        for _ in range(1, int(horizon)):
            f1 = omega + (alpha + beta) * f1
            forecasts.append(f1)
