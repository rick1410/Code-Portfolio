...
class GASMIDASNormalModel:
    """
    GAS-MIDAS with Normal innovations.
    ...
    inflation_series : np.ndarray
        Low-frequency series used as MIDAS regressor.
    ...
    inflation_series : np.ndarray
        Stored input series for MIDAS.
    inflation_dates : np.ndarray
        Dates for inflation series.
    daily_dates : np.ndarray
        Dates for daily returns.
    r : np.ndarray
        Return series aligned with K-lagged MIDAS block.
    T : int
        Effective sample size for likelihood (after alignment).
    X : np.ndarray
        MIDAS regressor matrix of shape (T, K-1).
    optimal_params : Optional[Dict[str, float]]
        Fitted parameters with keys: {"mu","alpha","beta","theta","w","m"}.
    ...
    standard_errors : Optional[np.ndarray]
        Approx. standard errors from inverse Hessian (when computed).
    """

    model_name: str = "GASMIDAS"
    ...
    def betapolyn(K: int, w: float) -> np.ndarray:
        """
        Unit-sum beta-polynomial MIDAS weights of length K-1.

        Parameters
        ----------
        K : int
            Total lags in block (effective regressors K-1).
        w : float
            Shape parameter (> 0).
        ...
        -------
        np.ndarray
            Weights of length K-1 summing to one.
        """
    ...
    def tau_series(self, theta: float, w: float, m: float) -> np.ndarray:
        ...
        w : float
            MIDAS weight-shape parameter (> 0).
        m : float
            Baseline level (>= 0).
    ...
    def log_likelihood(self, params: Sequence[float]) -> float:
        """
        Average negative log-likelihood under Gaussian innovations.
        ...
        for t in range(self.T - 1):
            sigma2_t = tau[t] * g[t]
            score = ((self.r[t] - mu) ** 2) / max(sigma2_t, 1e-12) - 1.0
            g[t + 1] = 1.0 + alpha * score * g[t] + beta * (g[t] - 1.0)
    ...
    def optimize(self, initial_params: Optional[Dict[str, float]] = None, compute_metrics: bool = False) -> Dict[str, float] | Tuple[Dict[str, float], float, float, float, np.ndarray]:
        """
        Fit parameters via SLSQP with constraints: alpha,beta in [0,1], alpha+beta≤1, theta,m≥0, w≥1.

        Parameters
        ----------
        initial_params : dict or None, default None
            If None, uses previous optimum if available else defaults:
            {'mu': mean(r), 'alpha': 0.1, 'beta': 0.85, 'theta': 0.1, 'w': 10, 'm': 0.001}.
        compute_metrics : bool, default False
            If True, compute total log-likelihood, AIC/BIC, and SEs.
        ...
        if initial_params is None:
            if self.optimal_params is not None:
                ...
                initial_params = {"mu": float(np.mean(self.log_returns)), "alpha": 0.1, "beta": 0.85, "theta": 0.1, "w": 10.0, "m": 0.001}
        ...
        if compute_metrics and self.convergence:
            self.log_likelihood_value = float(-res.fun * len(self.r))
            P = len(x0)
            self.aic, self.bic = self.compute_aic_bic(self.log_likelihood_value, P)

            H = approx_hess1(list(self.optimal_params.values()), self.log_likelihood, args=())
            cov = inv(H) / len(self.r)
            ...
            if np.isnan(self.standard_errors).any():
                ...
                self.standard_errors = np.sqrt(np.maximum(np.diag(cov_alt), 0.0))
    ...
    def multi_step_ahead_forecast(self, horizon: int) -> np.ndarray:
        """
        Multi-step variance forecasts for GAS-MIDAS (Normal).
        ...
        mu, alpha, beta, theta, w, m = list(self.optimal_params.values())
        ...
        for t in range(self.T):
            sigma2_t = tau[t] * g
            score = ((self.r[t] - mu) ** 2) / max(sigma2_t, 1e-12) - 1.0
            g = 1.0 + alpha * score * g + beta * (g - 1.0)
