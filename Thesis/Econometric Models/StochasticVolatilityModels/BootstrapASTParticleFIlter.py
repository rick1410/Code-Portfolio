...
class BootstrapASTFilterSV:
    """
    Bootstrap particle filter for a stochastic volatility model using an AST (asymmetric t) return density.

    The observation equation works with log-squared demeaned returns:
        x_t = log((r_t - \\bar{r})^2),
    and a linear-Gaussian latent log-variance state:
        alpha_{t+1} = psi + phi * alpha_t + eta_t,  eta_t ~ N(0, var_eta).

    A simple Kalman filter is used for Gaussian state filtering of `x_t`, and a bootstrap particle
    filter is used to form a filtered state path consistent with the AST return density.
    ...
    pdf_model : PDFModels
        Helper object providing AST pdf.
    ...
    demeaned_returns : np.ndarray
        r_t - mean_return.
    x_t : np.ndarray
        log((r_t - mean_return)^2) used as observation for the state filter.
    optimal_params : Optional[Dict[str, float]]
        Last optimized parameters if available, keys: {'psi','phi','var_eta'}.
    ...
    dD : float
        Constant offset in measurement equation (kept from the original, default -1.27).
    """

    model_name: str = "BFSV-AST"
    ...
    def kalman_filter(self, x: np.ndarray, psi: float, phi: float, var_eta: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """
        Run a univariate Kalman filter for the measurement `x_t = alpha_t + dD + eps_t`, eps_t ~ N(0, pi^2/2).
        ...
        phi : float
            AR(1) coefficient in the state equation.
        var_eta : float
        ...
        Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]
            (v, F, alpha, P, alpha_filt) where:
            v : prediction errors,
            F : prediction variances,
            alpha : predicted states (length T+1, alpha[0] is prior mean),
            P : predicted state variances (length T+1),
            alpha_filt : filtered state means at t=0..T-1.
        """
        T = len(x)
        v = np.zeros(T)
        F = np.zeros(T)
    ...
    def log_likelihood(self, x: np.ndarray, psi: float, phi: float, var_eta: float) -> float:
        """
        Gaussian log-likelihood (up to a constant) implied by the Kalman filter prediction errors.
    ...
    def objective(self, params: List[float], x: np.ndarray) -> float:
        """
        Wrapper objective with parameter domain checks for SLSQP.
    ...
    def moment_estimators(self) -> List[float]:
        """
        Obtain crude moment-based initial values (omega/psi, phi, var_eta).

        Returns
        -------
        List[float]
            [initial_omega (psi), initial_phi, initial_var_eta].
        """
        x = self.x_t
        initial_phi = float(np.corrcoef(x[:-1], x[1:])[0, 1])
        initial_omega = (1.0 - initial_phi) * (float(np.mean(x)) + 1.27)
        initial_var_eta = (1.0 - initial_phi**2) * (float(np.var(x)) - (np.pi**2) / 2.0)
        return [initial_omega, initial_phi, initial_var_eta]
    ...
    def optimize(self, initial_params: Optional[Dict[str, float]] = None, compute_metrics: bool = True) -> Union[Dict[str, float], Tuple[Dict[str, float], float, float, float, np.ndarray]]:
        """
        Estimate (psi, phi, var_eta) via SLSQP under bounds and return metrics if requested.
        ...
        compute_metrics : bool, default True
            If True, computes AIC, BIC, total log-likelihood, and standard errors.

        Returns
        -------
        Union[Dict[str, float], Tuple[Dict[str, float], float, float, float, np.ndarray]]
            If `compute_metrics` is True, returns (params, aic, bic, loglik, std_errs); else params.
        """
        x = self.x_t
        if initial_params is None:
            if self.optimal_params is not None:
                ...
                initial_psi, initial_phi, initial_var_eta = self.moment_estimators()
                initial_params = {"psi": initial_psi, "phi": initial_phi, "var_eta": initial_var_eta}
    ...
    def conditional_probability(self, y: float, H: float, xi: float) -> float:
        """
        One-step conditional probability contribution under AST for a given return and state.
        ...
        -------
        float
            Conditional density value under the standardized AST(0,1,delta=0.5,v1=3,v2=3).
        """
        sigma = float(np.exp((xi + H) / 2.0))
        y_standardized = y / sigma
        conditional_prob = PDFModels().ast_pdf(y_standardized, 0.0, 1.0, 0.5, 3.0, 3.0)
        return float(conditional_prob)

    def bootstrap(self) -> np.ndarray:
        """
        Bootstrap particle filter to obtain filtered states using AST measurement weights.

        Returns
        -------
        np.ndarray
            Filtered state means over time (length T).
        """
