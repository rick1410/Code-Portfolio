...
class GASModel:
    """
    Gaussian GAS(1,1) volatility model with score-driven update.
    ...
    optimal_params : Optional[Dict[str, float]]
        Estimated parameters with keys {'omega','alpha','beta'}.
    ...
    def log_likelihood(self, params: Sequence[float]) -> float:
        """
        Average negative log-likelihood for GAS(1,1) with Gaussian errors.
        ...
        for t in range(1, T):
            r_tm1 = r[t - 1]
            term1 = -0.5 + r_tm1**2
            term2 = 2.0 * float(np.exp(f[t - 1]))
            nabla_t = term1 / term2
            f[t] = omega + beta * f[t - 1] + alpha * nabla_t
        ll = -0.5 * np.log(2.0 * np.pi * np.exp(f)) - (r - mu_t) ** 2 / (2.0 * np.exp(f))
    ...
    def optimize(self, initial_params: Optional[Dict[str, float]] = None, compute_metrics: bool = False) -> Dict[str, float] | Tuple[Dict[str, float], float, float, float, np.ndarray]:
        """
        Estimate parameters via SLSQP with |beta|<1 bound-equivalent constraint.

        Parameters
        ----------
        initial_params : dict or None, default None
            If None, uses previous optimum else defaults
            {'omega': var(r[:50])*(1-0.1-0.7), 'alpha':0.1, 'beta':0.7}.
        ...
        if initial_params is None:
            if self.optimal_params is not None:
                ...
                initial_params = {"omega": v0 * (1.0 - 0.1 - 0.7), "alpha": 0.1, "beta": 0.7}

        keys: List[str] = list(initial_params.keys())
        x0: List[float] = list(initial_params.values())

        bounds = [(-100.0, 100.0), (0.0, 10.0), (0.0, 10.0)]
    ...
    def multi_step_ahead_forecast(self, horizon: int) -> np.ndarray:
        ...
        omega, alpha, beta = [self.optimal_params[k] for k in ["omega", "alpha", "beta"]]
        r = self.log_returns
        T = len(r)
        ...
        rT = r[-1]
        fT = f[-1]
        nabla_T = (-0.5 + rT**2) / (2.0 * float(np.exp(fT)))
        ...
        for _ in range(1, int(horizon)):
            ...
            nabla = (-0.5 + eps**2) / (2.0 * float(np.exp(f_prev)))
