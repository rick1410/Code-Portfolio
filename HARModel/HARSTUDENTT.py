...
class HARModelStudentT:
    """
    HAR (Heterogeneous Autoregressive) model with Student-t errors.
    ...
    def _har_mean_equation(self, t: int, params: Sequence[float]) -> float:
        """HAR mean μ_t = α0 + α1*RV_{t-1} + α2*RV^W_{t-1} + α3*RV^M_{t-1}.

        Parameters
        ----------
        t : int
            Current time index (t >= 1).
        ...
        """
        alpha0, alpha1, alpha2, alpha3, _, _ = params
    ...
    def log_likelihood(self, params: Sequence[float]) -> float:
        """Average negative log-likelihood under Student-t errors.

        Parameters
        ----------
        params : sequence of float
            (alpha0, alpha1, alpha2, alpha3, sigma2, nu1) with sigma2>0 and nu1>2.
        ...
        for t in range(22, T):
            mu_t = self._har_mean_equation(t, (alpha0, alpha1, alpha2, alpha3, sigma2, nu1))
        ...
        nll_i = const + 0.5 * np.log(sigma2) + 0.5 * (nu1 + 1.0) * np.log(1.0 + (res**2) / (nu1 * sigma2))
    ...
    def optimize(self, initial_params: Optional[Dict[str, float]] = None, compute_metrics: bool = False) -> Union[Dict[str, float], Tuple[Dict[str, float], float, float, float, np.ndarray]]:
        ...
        bounds = [(-10.0, 10.0), (-10.0, 10.0), (-10.0, 10.0), (-10.0, 10.0), (1e-12, 1e12), (2.001, 500.0)]
