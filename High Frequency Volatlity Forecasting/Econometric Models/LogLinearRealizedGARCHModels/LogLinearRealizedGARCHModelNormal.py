...
class LogLinearRealizedGARCHModelNormal:
    """
    Log-linear Realized GARCH(1,1) with Gaussian returns and Gaussian measurement equation.

    State (log variance):
        log h_t = omega + beta * log h_{t-1} + gamma1 * log x_{t-1}

    Returns block (Normal):
        r_t | h_t ~ N(0, h_t)
    ...
    def _avg_neg_loglik(self, par_list: List[float]) -> float:
        ...
        Parameter order (unconstrained, natural scale):
          [omega, beta, gamma1, xi, phi, tau_1, tau_2, sigma_u]
        """
        (omega, beta, gamma1, xi, phi, tau_1, tau_2, sigma_u) = [float(v) for v in par_list]

        # Basic parameter checks
        if sigma_u <= 0 or not np.isfinite(sigma_u):
            return 1e10
        if not np.all(np.isfinite([omega, beta, gamma1, xi, phi, tau_1, tau_2])):
            return 1e10
        ...
        for t in range(1, T):
            log_h[t] = omega + beta * log_h[t - 1] + gamma1 * np.log(x[t - 1])
        ...
        ll_r = -0.5 * (np.log(2.0 * np.pi) + np.log(h) + (r**2) / h)
    ...
    def optimize(self,initial_params: Optional[Dict[str, float]] = None,compute_metrics: bool = False) -> Union[Dict[str, float], Tuple[Dict[str, float], float, float, float, np.ndarray]]:
        ...
        MLE via SLSQP with bounds and a stability constraint:
            beta + phi * gamma1 < 1   (typical realized-GARCH stationarity condition)
        """
        if initial_params is None:
            if self.optimal_params is not None:
                ...
                initial_params = dict(
                    ...
                    beta=0.55,
                    gamma1=0.41,
                    xi=float(np.log(max(self.x.mean(), 1e-6))),  # a rough centering on log x
                    phi=1.04,
                    tau_1=-0.07,
                    tau_2=0.07,
                    sigma_u=0.38,
        ...
        # Stability: 1 - (beta + phi * gamma1) >= 0
        def _stab(pars: List[float]) -> float:
            d = dict(zip(keys, pars))
            return 1.0 - (d["beta"] + d["phi"] * d["gamma1"])
        ...
        if self.convergence:
            ...
            # return best known parameters (previous or initial)
    ...
    def multi_step_ahead_forecast(self, horizon: int) -> np.ndarray:
        ...
        gamma1 = float(self.optimal_params["gamma1"])
        ...
        for _ in range(int(horizon)):
            nxt = omega + beta * prev + gamma1 * lx_last
