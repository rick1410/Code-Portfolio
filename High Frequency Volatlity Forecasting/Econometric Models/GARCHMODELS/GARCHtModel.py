...
class GARCHtModel:
    ...
    optimal_params : Optional[Dict[str, float]]
        Fitted parameter dict with keys: {'omega','alpha','beta','nu1'}.
    ...
    def log_likelihood(self, params: Sequence[float]) -> float:
        """
        Average negative log-likelihood for GARCH(1,1)-t.
        ...
        for t in range(1, T):
            ...
            if sigma2[t] <= 0.0: return 1e6

        ll = studentt.logpdf(r, df=nu, loc=0.0, scale=np.sqrt(sigma2))
    ...
    def optimize(self, initial_params: Optional[Dict[str, float]] = None, compute_metrics: bool = False) -> Dict[str, float] | Tuple[Dict[str, float], float, float, float, np.ndarray]:
        """
        Fit parameters via SLSQP with positivity, stationarity, and ν>2 constraints.

        Parameters
        ----------
        initial_params : dict or None, default None
            If None, uses previous optimum if available else defaults:
            {'omega': var(r[:50])*0.2, 'alpha': 0.1, 'beta': 0.7, 'nu1': 3.0}.
        ...
        if initial_params is None:
            initial_params = self.optimal_params or {'omega': float(np.var(self.log_returns[:50]) * 0.2), 'alpha': 0.1, 'beta': 0.7, 'nu1': 3.0}
        keys = list(initial_params.keys()); x0 = list(initial_params.values())
        n_obs = len(self.log_returns)

        bounds = [(1e-6, None), (0.0, None), (0.0, None), (2.000001, None)]
        ...
        if compute_metrics and self.convergence:
            ...
            if not np.all(np.isfinite(ses)):
                ...
                grad = approx_fprime(np.asarray(res.x, dtype=float), self.log_likelihood, eps)
    ...
    def multi_step_ahead_forecast(self, horizon: int) -> np.ndarray:
        """
        Multi-step variance forecast via GARCH recursion under t-innovations.
        ...
        """
        if self.optimal_params is None: raise ValueError("Optimize model before forecasting.")
