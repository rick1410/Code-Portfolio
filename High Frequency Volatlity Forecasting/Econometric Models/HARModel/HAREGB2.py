...
class HARModelEGB2:
    """
    HAR model for realized volatility with EGB2 innovations.
    ...
    distribution : str
        Error distribution ('EGB2').
    only_kernel : bool
        Marker indicating the model uses only the kernel/volatility target.
    realized_volatility : np.ndarray
        Realized volatility series as float array.
    ...
    def _har_mean_equation(self, t: int, params: Sequence[float]) -> float:
        ...
        """
        alpha0, alpha1, alpha2, alpha3, _, _, _ = params
    ...
    def log_likelihood(self, params: Sequence[float]) -> float:
        """Average negative log-likelihood for HAR with EGB2 errors.
        ...
        log_lik = 0.0
        for t in range(22, T):
            mu_t = self._har_mean_equation(t, (alpha0, alpha1, alpha2, alpha3, sigma2, p, q))
            e_t = rv[t] - mu_t
            term = (0.5 * np.log(Omega)
                    + p * (np.sqrt(Omega) * e_t / np.sqrt(sigma2) + Delta)
                    - 0.5 * np.log(sigma2)
                    - np.log(gamma(p) * gamma(q) / gamma(p + q))
                    - (p + q) * np.log(1.0 + np.exp(np.sqrt(Omega) * e_t / np.sqrt(sigma2) + Delta)))
            log_lik += float(term)

        T_eff = T - 22
        if T_eff <= 0: return 1e10
        return float(-log_lik / T_eff)

    def compute_aic_bic(self, total_log_likelihood: float, num_params: int) -> Tuple[float, float]:
        """Compute AIC and BIC (preserving original scaling).

        Parameters
        ----------
        total_log_likelihood : float
            Total log-likelihood over the effective sample.
        num_params : int
        ...
        aic = 2.0 * num_params - 2.0 * total_log_likelihood
        bic = np.log(T) * num_params - 2.0 * total_log_likelihood
    ...
    def optimize(self, initial_params: Optional[Dict[str, float]] = None, compute_metrics: bool = False) -> Union[Dict[str, float], Tuple[Dict[str, float], float, float, float, np.ndarray]]:
        """Estimate parameters by SLSQP under bounds.
        ...
        bounds = [(-10.0, 10.0), (-10.0, 10.0), (-10.0, 10.0), (-10.0, 10.0), (1e-3, 100.0), (1e-3, 100.0), (1e-3, 100.0)]
