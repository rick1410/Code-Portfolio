...
class GARCHMIDASEGB2Model:
    """
    GARCH-MIDAS with EGB2 innovations.
    ...
    model_name: str = "GARCHMIDAS-EGB2"
    ...
    def _egb2_constants(self, p: float, q: float) -> Tuple[float, float, float]:
        """
        EGB2 helper constants (Δ, Ω, norm const).

        Parameters
        ----------
        p, q : float
            Shape parameters (> 2 for finite variance of implied errors).

        Returns
        -------
        (float, float, float)
            (Delta, Omega, normalization_constant).
        """
        Delta = float(digamma(p) - digamma(q))
        Omega = float(polygamma(1, p) + polygamma(1, q))
        norm_const = float(sp_gamma(p) * sp_gamma(q) / sp_gamma(p + q))
        return Delta, Omega, norm_const

    def log_likelihood(self, params: Sequence[float]) -> float:
        """
        Average negative log-likelihood under EGB2 innovations.

        Parameters
        ----------
        params : sequence of float
            (mu, alpha, beta, theta, w, m_param, p, q).
        ...
        """
        mu, alpha, beta, theta, w, m_param, p, q = params
    ...
    def optimize(self, initial_params: Optional[Dict[str, float]] = None, compute_metrics: bool = False) -> Dict[str, float] | Tuple[Dict[str, float], float, float, float, np.ndarray]:
        ...
        initial_params : dict or None, default None
            ...
            {'mu': mean(log_returns), 'alpha': 0.1, 'beta': 0.85,
             'theta': 0.1, 'w': 10, 'm_param': 0.001, 'p': 2.0, 'q': 2.0}
        ...
        if initial_params is None:
            initial_params = self.optimal_params if self.optimal_params is not None else {'mu': float(np.mean(self.log_returns)), 'alpha': 0.1, 'beta': 0.85, 'theta': 0.1, 'w': 10.0, 'm_param': 0.001, 'p': 2.0, 'q': 2.0}
        keys = list(initial_params.keys()); x0 = np.array(list(initial_params.values()), dtype=float)
        bounds = [(-np.inf, np.inf), (0.0, 1.0), (0.0, 1.0), (0.0, None), (1.0, None), (0.0, None), (2.000001, None), (2.000001, None)]
    ...
    def multi_step_ahead_forecast(self, horizon: int) -> np.ndarray:
        """
        Multi-step-ahead variance forecasts using GARCH-MIDAS recursion (EGB2 errors).
