...
class BootstrapFilterNormalSV:
    """
    Bootstrap particle filter for a stochastic volatility model with Gaussian returns.

    The observation driving the state filter is the log-squared demeaned return:
        x_t = log((r_t - \bar{r})^2).
    ...
    particle filter supplies filtered states with Normal measurement weights for returns.
    ...
    @staticmethod
    def conditional_probability(y: float, H: float, xi: float) -> float:
        """
        One-step conditional probability under Normal returns for a given return and state.
        ...
        -------
        float
            Conditional density value under N(0, sigma^2) with sigma^2 = exp(xi + H).
        """
        sigma = float(np.exp((xi + H) / 2.0))
        return float(1.0 / (sigma * np.sqrt(2.0 * np.pi)) * np.exp(-0.5 * (y / sigma) ** 2))

    def bootstrap(self) -> np.ndarray:
        """
        Bootstrap particle filter to obtain filtered states using Normal measurement weights.
