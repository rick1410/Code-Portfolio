...
class BootstrapStudenttFilterSV:
    """
    Bootstrap particle filter for a stochastic volatility model with Student-t return density in weighting.

    The observation used to filter the latent log-variance is
        x_t = log((r_t - \bar{r})^2),
    with a Gaussian linear state-space for x_t and an AR(1) latent log-variance state:
    ...
    pdf_model : PDFModels
        PDF helper for densities.
    log_returns : np.ndarray
        Input return series r_t.
    mean_return : float
        Sample mean of returns.
    demeaned_returns : np.ndarray
        r_t - mean_return.
    x_t : np.ndarray
        log((r_t - mean_return)^2).
    optimal_params : Optional[Dict[str, float]]
        Optimized parameters (keys: 'psi','phi','var_eta'), if available.
    ...
    dD : float
        Constant offset in the measurement equation of the Kalman filter.
    """

    model_name: str = "BFSV-t"
    distribution: str = "Normal"

    def __init__(self, log_returns: np.ndarray) -> None:
        """
        Initialize data containers and precompute auxiliary series.
    ...
    def kalman_filter(self, x: np.ndarray, psi: float, phi: float, var_eta: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """
        Run a univariate Kalman filter for x_t = alpha_t + dD + eps_t with eps_t ~ N(0, pi^2/2).

        Parameters
        ----------
        x : np.ndarray
            Observed series (log-squared demeaned returns), shape (T,).
        psi : float
            State intercept.
        phi : float
            State AR(1) coefficient.
        var_eta : float
    ...
    def objective(self, params: List[float], x: np.ndarray) -> float:
        """
        Objective with domain checks for (psi, phi, var_eta).
    ...
    def optimize(self, initial_params: Optional[Dict[str, float]] = None, compute_metrics: bool = True) -> Union[Dict[str, float], Tuple[Dict[str, float], float, float, float, np.ndarray]]:
        """
        Estimate (psi, phi, var_eta) by SLSQP with bounds; optionally compute AIC/BIC/SEs.
    ...
    def conditional_probability(self, y: float, H: float, xi: float) -> float:
        """
        Student-t conditional density for the return given latent state and long-run component.
        ...
        -------
        float
            Student-t density at y with scale derived from exp(xi + H).
        """
        v = 5
        sigma = float(np.exp((xi + H) / 2.0))
        return float(PDFModels().student_t_pdf(y, mu_t=0, h_t=sigma, v=v))

    def bootstrap(self) -> np.ndarray:
        """
        Bootstrap particle filter to obtain filtered states using Student-t measurement weights.
    ...
    def multi_step_ahead_forecast(self, horizon: int) -> np.ndarray:
        ...
        var_eta = float(self.optimal_params["var_eta"])  # acknowledged but unused in deterministic propagation
        _ = var_eta
        kappa = psi / (1.0 - phi)
