...
class LogHARModel:
    """
    Log-HAR (Heterogeneous AutoRegressive) model with Gaussian errors on log-realized volatility.
    ...
    The residuals e_t = y_t - E[y_t | F_{t-1}] are assumed Gaussian with variance sigma2.
    ...
    def log_likelihood(self, params: List[float]) -> float:
        """
        Average *negative* log-likelihood for Log-HAR with Gaussian errors.
        ...
        T = len(y)
        if T < 23:
            # need 22 obs to form lags
            return 1e10

        # Build residuals from t=22 onward (first 22 obs used for lags)
        resid = np.array(
        ...
        # Gaussian average negative log-likelihood
        nll = 0.5 * (np.log(2.0 * np.pi) + np.log(sigma2) + (resid**2) / sigma2)
    ...
    def optimize(self,initial_params: Optional[Dict[str, float]] = None,compute_metrics: bool = False) -> Union[Dict[str, float], Tuple[Dict[str, float], float, float, float, np.ndarray]]:
        ...
        initial_params : dict or None
            Keys: alpha0, alpha1, alpha2, alpha3, sigma2.
        ...
        # Simple bounds; no sum constraint needed for HAR
        bounds = [
            (-10.0, 10.0),  # alpha0
            (-10.0, 10.0),  # alpha1
            (-10.0, 10.0),  # alpha2
        ...
        if self.convergence:
            ...
            # Keep prior parameters if available, otherwise return the starting point
            return self.optimal_params if self.optimal_params is not None else dict(zip(keys, x0))

        if compute_metrics and self.convergence:
            # res.fun is average NEGATIVE LL over effective sample (T_eff = T-22)
